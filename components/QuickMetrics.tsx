...
function MetricCard({ label, value, positive, sub }: MetricCardProps) {
  return (
    <div className="bg-[var(--surface-muted)] rounded-lg p-4 border border-[var(--border)] text-center">
      ...
      <div className={`text-2xl sm:text-3xl font-bold font-mono ${positive ? 'text-success' : 'text-error'}`}>
        {value}
      </div>
    ...
    <Section title="Rental Investment Metrics">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-[var(--surface-muted)] rounded-lg p-4 border border-[var(--border)] text-center">
          ...
          <div className={`text-2xl sm:text-3xl font-bold ${passesOnePercentRule ? 'text-success' : 'text-error'}`}>
