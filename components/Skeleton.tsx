...
export function StatSkeleton() {
  return (
    <div className="bg-surface rounded-lg p-3 md:p-4 border border-border min-w-0 animate-pulse">
...
export function ChartSkeleton({ height = 256 }: { height?: number }) {
  return (
    <div
      className="animate-pulse bg-surface-muted rounded-xl border border-border flex items-end justify-center gap-2 p-4"
      style={{ height }}
    >
      ...
      {Array.from({ length: 12 }).map((_, i) => (
        <div
          key={i}
          className="bg-border rounded-t flex-1 transition-all duration-500"
          style={{
        ...
        <div className="ml-8 h-48 md:h-64 flex items-end gap-[1px] md:gap-0.5 p-4">
          {Array.from({ length: 25 }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className="flex-1 bg-surface-muted rounded-t transition-all duration-300"
                style={{
                  height: `${height}%`,
...
export function LoadingSpinner({ message = 'Running simulation...' }: { message?: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 animate-in fade-in duration-200">
      ...
      <p className="mt-4 text-content-muted text-sm animate-pulse">{message}</p>
    </div>
  )
}
